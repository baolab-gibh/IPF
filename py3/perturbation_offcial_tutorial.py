...
amp = True
load_model = "../inputs/scGPT/pretrained_models/whole_blood"
...
perts_to_plot = {"norman": ["SAMD1+ZBTB1"], "adamson": ["KCTD16+ctrl"]}.get(data_name)
# if data_name == "norman": perts_to_plot = ["SAMD1+ZBTB1"]
# elif data_name == "adamson": perts_to_plot = ["KCTD16+ctrl"]
...
if load_model is not None:
    model_dir = Path(load_model).resolve()
...
if load_param_prefixs is not None and load_model is not None:
    ...
    pretrained_dict = {
        ...
        if any([k.startswith(prefix) for prefix in load_param_prefixs]) and k in model_dict
    }
    for k, v in pretrained_dict.items():
...
def plot_perturbation(model: nn.Module, query: str, save_file: str = None, pool_size: int = None):
